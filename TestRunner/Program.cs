...
// RazorRenderer test harness.
//
//   dotnet run --project TestRunner                      run everything
//   dotnet run --project TestRunner -- OnClick           run tests whose name contains "OnClick"
//   dotnet run --project TestRunner -- --headed          watch it in a real browser window
//   dotnet run --project TestRunner -- --url http://...  use an already running dev server
//
...
var hostProject = Path.Combine(repoRoot, "WasmTestHost", "WasmTestHost.csproj");
if (!File.Exists(hostProject))
{
    Console.Error.WriteLine($"Could not find WasmTestHost.csproj (looked in {hostProject})");
    return 1;
}
...
try
{
    ...
    {
        (server, url) = await StartServerAsync(hostProject);
        if (string.IsNullOrEmpty(url))
        {
...
static async Task<(Process?, string)> StartServerAsync(string hostProject)
{
    Console.WriteLine("building and starting WasmTestHost...");
    var psi = new ProcessStartInfo("dotnet", $"run -c Release --project \"{hostProject}\"")
    {
...
static async Task<int> RunAsync(string url, bool headed, bool verbose)
{
    ...
    // Navigate only until the document is parsed - NOT network-idle. The real completion signal is the
    // page's own "RESULTS:" console line, awaited via finished.Task below.
